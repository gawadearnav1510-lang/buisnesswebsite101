...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // toast is already handled inside context
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="pt-16 min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-zinc-950 px-6">
      <div className="max-w-md w-full bg-white dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-800/50 rounded-[40px] p-8 md:p-12 shadow-xl shadow-zinc-100 dark:shadow-none">
        ...
        <div className="text-center space-y-3 mb-10">
          ...
          <p className="text-sm text-zinc-500">Sign in to manage your partnerships &amp; invoices</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className="block text-xs font-bold uppercase tracking-[1px] text-zinc-400">Password</label>
              <Link to="/forgot-password" className="text-xs text-violet-600 dark:text-violet-400 font-semibold hover:underline">Forgot password?</Link>
            </div>
            <div className="relative">
              <span className="absolute inset-y-0 left-0 flex items-center pl-4.5 pointer-events-none text-zinc-400">
        ...
        <div className="pt-8 mt-8 border-t border-zinc-100 dark:border-zinc-800 text-center">
          <p className="text-xs text-zinc-500">
            Don't have an account yet? <Link to="/register" className="text-violet-600 dark:text-violet-400 font-semibold hover:underline">Create an account</Link>
          </p>
        </div>
